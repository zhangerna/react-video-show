...
class DownloadFilter extends React.Component {
	render(){
		return (
			<div className="Filter-martin">
				<span>状态</span>
				<ul className="content-martin">
					{
						this.props.download_status.map((ele, index) => (
							this.props.current_download_status == index ?
								<li className="red-inline" key={index} value={index} onClick={this.props.getDownloadFilter}>{ele}</li>
							:	
								<li className="inline" key={index} value={index} onClick={this.props.getDownloadFilter}>{ele}</li>
						))
					}
				</ul>
			</div>
		...
		return (
			<div>
				...
				<div>
					...
					<DownloadFilter current_download_status={this.state.current_download_status} download_status={this.state.download_status} getDownloadFilter={this.getDownloadFilter} />
